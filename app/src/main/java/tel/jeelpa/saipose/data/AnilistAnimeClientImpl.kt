...
class AnilistAnimeClientImpl(
    private val apolloClient: ApolloClient
) : AnimeClient {
    override suspend fun getTrending(): List<BaseAnime> {
        return apolloClient.query(AnimeBaselineQuery(
            page = 1,
            perPage = 50,
    ...
    override suspend fun getPopular(): List<BaseAnime> {
        return apolloClient.query(AnimeBaselineQuery(
            page = 1,
            perPage = 50,
    ...
    override suspend fun getRecentlyUpdated(): List<BaseAnime> {
    ...
    override suspend fun getAnimeDetails(id: Int): AnimeDetails {
        ...
        val data = apolloClient.query(AnimeDetailsQuery(id = id)).execute().data!!.Media!!
    ...
    override suspend fun searchAnime(query: String): List<BaseAnime> {
        return apolloClient.query(AnimeSearchQuery(search = query)).execute()
            .data?.Page?.media?.mapNotNull {
                // return if media is null
                it ?: return@mapNotNull null
                val title = it.title
