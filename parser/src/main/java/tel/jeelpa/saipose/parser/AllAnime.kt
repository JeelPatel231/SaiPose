...
import tel.jeelpa.saipose.reference.Subtitle
...
import tel.jeelpa.saipose.reference.adult
...
class AllAnime : AnimeParser() {
    ...
    private val allAnimeDirectExtractor = AllAnimeDirectExtractor()
    private val allAnimeExtractor = AllAnimeExtractor()

    override suspend fun extractVideo(server: VideoServer): VideoContainer? {
        if (server.extraData?.get("type") == "player")
            return allAnimeDirectExtractor.extract(server)
        ...
        val extractor = when {
            ...
            "apivtwo" in path   -> allAnimeExtractor
            else                -> null
        }
    ...
    private val episodeInfoHash = "21c54fb01a8de305bbf70d48d879438f195e3e51da2e95a09ac57c77babb1030" //
    ...
    private class AllAnimeExtractor : VideoExtractor() {
        override suspend fun extract(server: VideoServer): VideoContainer {
            ...
            val vid = res.links?.asyncMapNotNull { i ->
                ...
                when {
                    i.crIframe == true -> {
                        i.portData?.streams?.asyncMapNotNull {
                            when {
                                ...
                                ->
                                    Video(
                                        null,
                                        VideoType.DASH,
                                        it.url ?: return@asyncMapNotNull null,
                                        null,
                                        "DASH"
                                    )
                    ...
                    i.hls == true -> listOf(
                        Video(
                            null,
                            VideoType.M3U8,
                            i.link ?: return@asyncMapNotNull null,
                            null,
                            i.resolutionStr
                        )
                    )
            ...
            return VideoContainer(vid, sub)
        }
    }

    private class AllAnimeDirectExtractor : VideoExtractor() {
        override suspend fun extract(server: VideoServer): VideoContainer {
            val url = server.embed.url
            return VideoContainer(listOf(Video(null, VideoType.CONTAINER, url, getSize(url))))
        }
    }
